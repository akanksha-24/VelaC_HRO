...
class HRO:
    ...
    def __init__(self, Map, Qmap=None, Umap=None, Bmap=None, hdu=None, vecMask=None, msk=False, kernel='Gaussian', kstep=1, convention=1, gstep=[1,1,1], histbins=20, compare=False, Qmap2=None, Umap2=None, BinMap=None):
        '''Intialization of HRO class'''

        # Initialize
        self.unmasked_Map = Map if Map is None else Map.copy()
        self.Map = Map if Map is None else Map.copy()                                             # Original data before masking
        ...
        self.BinMap = Map.copy() if BinMap is None else BinMap.copy()
    ...
    def HROanalysis(self, convention, msk, hdu, vecMask, gstep, kstep, kernel, histbins):
        ...
        if (self.Qmap is None)==False:
            ...
            assert self.Map.shape == self.Bmap.shape, "Dimensions of Map and Polarization data do not match"
            ...
            # B-field is given
            self.Bfield = self.Bmap                                           # Inferred B-field is 90 deg offset from polarization angle
            self.Bx = np.cos(self.Bfield) 
            self.By = np.sin(self.Bfield)
